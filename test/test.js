	...
	describe('cameras', () => {
        ...
        describe('DJI Zenmuse XT2 640 R 13mm', () => {
        	...
        	it('RGB', async () => {
				...
				assert.deepEqual(exif.ModifyDate, new Date('2019-05-24T15:24:01.000Z'))
				...
				assert.deepEqual(exif.DateTimeOriginal, new Date('2019-05-24T15:24:01.000Z'))
				assert.deepEqual(exif.DateTimeDigitized, new Date('2019-05-24T15:24:01.000Z'))
		...
		describe('FLIR DUO PRO 640 R 13mm', () => {
			...
			it('TRM', async () => {
				...
				assert.equal(exif.FNumber, 1.25)
				assert.deepEqual(exif.DateTimeOriginal, new Date('2019-06-14T12:01:23.000Z'))
				assert.equal(exif.FocalLength, 13)
				assert.equal(exif.ImageNumber, 0)
		...
		it('MicaSense RedEdge M', async () => {
			...
			assert.deepEqual(exif.ModifyDate, new Date('2018-09-02T12:21:05.000Z'))
			...
			assert.deepEqual(exif.DateTimeOriginal, new Date('2018-09-02T12:21:05.000Z'))
			assert.deepEqual(exif.DateTimeDigitized, new Date('2018-09-02T12:21:05.000Z'))
		...
		it('Delair UX11', async () => {
            ...
            assert.equal(exif.ResolutionUnit, 2)
            assert.deepEqual(exif.ModifyDate, new Date('2019-03-14T22:45:00.000Z'))
            assert.equal(exif.YCbCrPositioning, 1)
            ...
            assert.equal(exif.ExifVersion, '0230')
            assert.deepEqual(exif.DateTimeOriginal, new Date('2019-03-14T22:45:00.000Z'))
            assert.deepEqual(exif.DateTimeDigitized, new Date('2019-03-14T22:45:00.000Z'))
        ...
        it('FLIR Tau 2 640 R 19mm', async () => {
			...
			assert.deepEqual(exif.ModifyDate, new Date('2019-07-26T09:00:39.000Z'))
			assert.deepEqual(exif.DateTimeOriginal, new Date('2019-07-19T11:21:52.000Z'))
			assert.equal(exif.GPSLatitudeRef, 'N')
			...
			assert.deepEqual(exif.timestamp, new Date('2019-07-19T11:21:52.000Z'))
		...
		it('Sensefly Thermomap 9mm', async () => {
			...
			assert.deepEqual(exif.DateTimeOriginal, new Date('2015-11-19T11:35:28.000Z'))
		...
		it('Wiris 2nd Gen 640 19mm', async () => {
			...
			assert.deepEqual(exif.ModifyDate, new Date('2017-09-16T00:00:00.000Z'))
			assert.equal(exif.SampleFormat, 1)
			assert.equal(exif.SMinSampleValue, 7296)
			assert.equal(exif.SMaxSampleValue, 7774)
			assert.equal(exif.CameraSerialNumber, '170307')
			assert.deepEqual(exif.DateTimeOriginal, new Date('2017-09-16T15:28:54.000Z'))
			...
			assert.deepEqual(exif.timestamp, new Date('2017-09-16T15:28:54.000Z'))
